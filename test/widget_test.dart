// This is a basic Flutter widget test.
//
// To perform an interaction with a widget in your test, use the WidgetTester
// utility that Flutter provides. For example, you can send tap and scroll
// gestures. You can also use WidgetTester to find child widgets in the widget
// tree, read text, and verify that the values of widget properties are correct.

import 'package:flutter_test/flutter_test.dart';
import 'package:onoo/src/data/model/auth/auth_model.dart';
import 'package:onoo/src/data/model/discover/discover_model.dart';
import 'package:onoo/src/data/repository.dart';

void main() {
  test('Counter increments smoke test', () async {
    //ConfigData? configData = await Repository().getConfigData();
    //HomeContent? homeContent = await Repository().getHomeContent();
    //VideoContent? videoContent = await Repository().getVideoContent();
    // DiscoverModel? discoverModel = await Repository().getDiscoverData();
    AuthModel? authModel = await Repository().login(email: "mannanzinat@gmail.com",password: "123456");
    print(authModel!.success);
  });
}
